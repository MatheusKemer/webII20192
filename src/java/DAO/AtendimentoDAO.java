...
public class AtendimentoDAO {
    ...
    public List<Atendimento> buscarPorUsuario(int id){
        ...
        try {
            con = ConnectionFactory.getConnection();
            st = con.prepareStatement("SELECT id_atendimento, dt_hr_atendimento, dsc_atendimento, nome_cliente, nome_produto FROM tb_atendimento, tb_produto, tb_cliente, tb_usuario WHERE tb_usuario.id_usuario = ? AND tb_atendimento.id_usuario = tb_usuario.id_usuario AND tb_atendimento.id_cliente = tb_cliente.id_cliente AND tb_produto.id_produto = tb_atendimento.id_produto");
            st.setInt(1, id);
    ...
    public void inserir(Atendimento atendimento){
        ...
        try {
            con = ConnectionFactory.getConnection();
            st = con.prepareStatement("INSERT INTO tb_atendimento (dt_hr_atendimento, dsc_atendimento, id_produto, id_cliente, id_usuario, id_tipo_atendimento, res_atendimento) VALUES (?, ?, ?, ?, ?, ?, ?)");
            st.setDate(1, (Date) atendimento.getData());
            st.setString(2, atendimento.getDesc());
            st.setInt(3, atendimento.getProdutoId());
            st.setInt(4, atendimento.getClienteId());
            st.setInt(5, atendimento.getUsuarioId());
            st.setInt(6, atendimento.getTipoAtendimentoId());
            st.setString(7, String.valueOf(atendimento.getStatus()));
